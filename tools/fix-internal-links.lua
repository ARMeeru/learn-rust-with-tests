-- Ported from fix-internal-links.lua in Learn Go with Tests by Chris James
-- (MIT). The only change: the list of chapter files is taken from pandoc's
-- input files instead of being typed in, so tools/build-books.sh stays the
-- single place that decides what goes in the ebook.
--
...
-- The markdown files passed to pandoc (see tools/build-books.sh). Used to
-- whitelist which "#{prefix}" / "{file}.md" links are genuinely internal
-- chapter references, so we don't misfire on something that merely looks
-- similar.
local knownFiles = {}
for _, path in ipairs(PANDOC_STATE.input_files) do
  table.insert(knownFiles, (path:match("([^/]+)$") or path))
end
...
local externalOverrides = {
  ["LICENSE.md"] = "https://github.com/ARMeeru/learn-rust-with-tests/blob/main/LICENSE.md",
}
